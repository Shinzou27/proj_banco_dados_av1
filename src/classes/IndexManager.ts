...
class IndexManager {
  ...
  constructor(
    ...
    this.outputStream = fs.createWriteStream("src/public/words-pages.txt", {
      flags: "a",
    });
  }

  async loadFile(data: string[]): Promise<void> {
  ...
  private async saveWordPageRelation(
    word: string,
    pageNumber: number
  ): Promise<void> {
  ...
  static async loadFileData(): Promise<string[]> {
    try {
      const data = fs.readFileSync(
        path.resolve("src/public/words.txt"),
        "utf-8"
      );
      const dataFormatt = data
        .split("\n")
        .map((line) => line.trim())
        .filter(Boolean); // Remove espaços e linhas vazias
  ...
  indexSearch(searchWord: string): {
    pageFound: number | undefined;
    pagesScanned: number;
    timeElapsed: number;
    pageContent: string[];
  } {
  ...
  buildIndex(): void {
    this.pages.forEach((page) => {
      page.words.forEach((word) => {
        const { collision, overflow } = this.hashIndex.storeInBucket(
          word,
          page.pageNumber
        );
        this.numCollisions += collision;
